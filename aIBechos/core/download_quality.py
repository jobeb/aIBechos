...
# Proveedores (ver set_provider_lists). _BUILTIN_TRUSTED son los valores
# por defecto (MISMA lista que config.py::DEFAULTS -- mantener
# sincronizadas) y el estado inicial del módulo, para que el scoring
# funcione igual aunque nadie haya llamado al setter (tests, watcher
# standalone). La lista del box es AUTORITATIVA: si el usuario quita
# "grupots", deja de puntuar (no hay bonus escondido).
_BUILTIN_TRUSTED = ["exploradoresp2p", "grupots", "hispashare",
                    "hispashare.org", "nocturniap2p"]

# Vetos de idioma fijos del sistema (ver _VOS_RE/_FRA_RE/_ITA_RE/_GER_RE/
# _POR_RE y sus is_*_content): siempre activos, no editables. Solo
# informativo: se muestra como referencia (líneas "#") en Ajustes →
# Servidor → Proveedores. Si el motor gana un idioma, añadirlo aquí.
...
def parse_provider_lines(text: str) -> list:
    ...
    comentarios ("#" al inicio: ahí van los vetos fijos de referencia).
    Conserva la grafía escrita para mostrarla. Pura, testeable."""
    out = []
    seen = set()
    try:
...
def set_provider_lists(trusted=None, blocked=None) -> None:
    ...
    - *trusted*: lista COMPLETA de grupos de confianza (reemplaza a los
      built-in, no se suma). None = no tocar.
    - *blocked*: marcas EXTRA bloqueadas (los vetos de idioma del motor
      siguen aparte, siempre activos). None = no tocar.
...
def is_trusted_provider(name: str) -> bool:
    """True si el nombre contiene algún grupo de la lista efectiva de
    confianza (built-in o editada en Ajustes). Mismo bonus que antes."""
...
def is_german_content(name: str) -> bool:
    ...
    excluye, igual que el italiano solo (ver is_italian_only). Un dual
    con castellano no se excluye (tiene español), solo se penaliza."""
    if not name or _LANG_RE.search(name):
        return False
    return bool(_GER_RE.search(name))
...
def is_portuguese_content(name: str) -> bool:
    ...
    español: se excluye, igual que el italiano solo. Un dual con
    castellano no se excluye, solo se penaliza."""
    if not name or _LANG_RE.search(name):
        return False
    return bool(_POR_RE.search(name))
...
def score_download(result: AmuleSearchResult,
    ...
    *expected_year* es el año de una película (el botón ⬇ de Películas busca
    ...
    if is_german_only(name) or is_portuguese_only(name):
        return 0.0
    # Marca extra bloqueada por el usuario en Ajustes (ver
    # set_user_provider_lists/is_user_blocked).
    ...
    if _ITA_RE.search(name):
        score -= 60.0
    # Alemán/portugués con español (dual): -60 igual que el italiano (el
    # "solo X" ya se excluyó arriba y nunca llega aquí con nota).
    if _GER_RE.search(name):
        score -= 60.0
    if _POR_RE.search(name):
        score -= 60.0
    ...
    # Grupos de confianza (lista efectiva: built-in o editada en Ajustes →
    # Servidor → Proveedores, ver trusted_provider_match).
...
def explain_score(result: AmuleSearchResult, query: str = "", expected_year: int = None,
    ...
    if _GER_RE.search(name):
        lines.append("Alemán GER → excluido" if is_german_only(name) else "Alemán GER → -60")
    if _POR_RE.search(name):
        lines.append("Portugués → excluido" if is_portuguese_only(name) else "Portugués → -60")
    if is_vos_content(name) or is_french_content(name):
    ...
    if _trust_hit:
        ...
        _glabel = ("GrupoTS/hispashare" if _trust_hit.lower() in _BUILTIN_TRUSTED
                   else "proveedor de confianza (lista propia)")
        if _sz and typical_size and not is_movie:
            _ratio = _sz / typical_size if typical_size else 1
            if _ratio < 0.5 or _ratio > 2.0:
                lines.append(f"{_glabel} → +5 (outlier tamaño, no +25)")
            else:
                lines.append(f"{_glabel} → +25")
        else:
            lines.append(f"{_glabel} → +25")
    # Fuentes
    s = int(result.sources or 0)
