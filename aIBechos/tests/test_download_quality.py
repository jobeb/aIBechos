...
def test_french_bare_fr_tag_excluded_doctor_who():
    """Caso real: Doctor Who 3x12 con tag FR suelto se elegía porque
    _FRA_RE no cubría el FR sin más (solo VOSTFR/VF/FRENCH)."""
    name = "Doctor.Who.2005.3x12.FR.720p.HDTV.x264.mkv"
    assert is_french_content(name)
    assert score_download(_r(1, name, size="800 MB", sources=50, complete=True),
                          "Doctor Who 2005 3x12") == 0.0
    assert best_result([_r(1, name, size="800 MB", sources=50, complete=True)],
                       "Doctor Who 2005 3x12") is None


def test_french_tag_variants_excluded():
    for tag in ("TRUEFRENCH", "VFF", "VQ", "F.R.", "[FR]", "VOSTFR"):
        name = f"Serie 1x02 {tag} 720p.mkv"
        assert is_french_content(name), tag
        assert best_result([_r(1, name)], "Serie 1x02") is None, tag


def test_fr_inside_normal_words_not_flagged():
    """"fr" dentro de palabras normales no marca francés."""
    assert not is_french_content("Franklin 1x01 La verdad 720p.mkv")
    assert not is_french_content("Alfred.Hitchcock.2x03.720p.mkv")
    assert not is_french_content("Serie 1x01 720p castellano.mkv")


def test_french_with_spanish_stays_eligible():
    """Francés + castellano (dual/subs): no se excluye, como el italiano
    dual — el español manda."""
    name = "Serie 3x12 FRENCH Spanish subs 720p.mkv"
    assert not is_french_content(name)
    assert best_result([_r(1, name)], "Serie 3x12") is not None


def test_french_never_beats_spanish():
    """Aunque el FR tenga más fuentes y resolución, el español gana."""
    fr = _r(1, "Doctor.Who.2005.3x12.FR.1080p.HDTV.x264.mkv",
            size="1500 MB", sources=100, complete=True)
    es = _r(2, "Doctor.Who.2005.3x12.720p.HDTV.x264-GrupoTS.mkv",
            size="800 MB", sources=5, complete=True)
    assert best_result([fr, es], "Doctor Who 2005 3x12").number == 2


def test_german_only_excluded_dual_penalized():
    assert is_german_only("Serie 4x04 GERMAN 1080p.mkv")
    assert is_german_only("Serie 4x04 -GER- 720p.mkv")
    assert best_result([_r(1, "Serie 4x04 GERMAN 1080p.mkv")], "Serie 4x04") is None
    # Con castellano sigue elegible pero no gana al español puro.
    dual = _r(1, "Serie 4x04 GERMAN castellano 1080p.mkv", size="1 GB", sources=50, complete=True)
    assert not is_german_only(dual.name)
    es = _r(2, "Serie 4x04 castellano 720p.mkv", size="500 MB", sources=5, complete=True)
    assert best_result([dual, es], "Serie 4x04").number == 2


def test_portuguese_only_excluded_dual_stays_eligible():
    assert is_portuguese_only("Serie 5x01 DUBLADO 720p.mkv")
    assert is_portuguese_only("Serie 5x01 PORTUGUESE 720p.mkv")
    assert best_result([_r(1, "Serie 5x01 DUBLADO 720p.mkv")], "Serie 5x01") is None
    dual = "Serie 5x01 DUBLADO castellano 720p.mkv"
    assert not is_portuguese_only(dual)
    assert best_result([_r(1, dual)], "Serie 5x01") is not None


def test_ger_por_inside_normal_words_not_flagged():
    assert not is_german_only("Serie 1x01 El gerente 720p.mkv")
    assert not is_portuguese_only("Serie 1x01 El puerto 720p.mkv")


def test_user_trusted_group_bonus():
    """Un grupo extra de confianza (Ajustes → Servidor → Proveedores)
    puntúa como los built-in."""
    from core.download_quality import (
        set_provider_lists, is_trusted_provider, _BUILTIN_TRUSTED)
    set_provider_lists(trusted=[*_BUILTIN_TRUSTED, "migrupo"], blocked=[])
    try:
        assert is_trusted_provider("Serie 1x01 MiGrupo 720p.mkv")
        a = _r(1, "Serie 1x01 migrupo 720p.mkv", size="500 MB", sources=5, complete=True)
        b = _r(2, "Serie 1x01 otrogrupo 720p.mkv", size="500 MB", sources=5, complete=True)
        assert score_download(a, "Serie 1x01") > score_download(b, "Serie 1x01")
        assert best_result([b, a], "Serie 1x01").number == 1
    finally:
        set_provider_lists(trusted=list(_BUILTIN_TRUSTED), blocked=[])


def test_trusted_list_is_authoritative():
    """La lista del box MANDA: quitar 'grupots' le quita el bonus (no hay
    bonus escondido), y el match lleva guardas ('xgrupots' no vale)."""
    from core.download_quality import (
        set_provider_lists, is_trusted_provider, _BUILTIN_TRUSTED)
    set_provider_lists(trusted=["migrupo"], blocked=[])
    try:
        assert not is_trusted_provider("Serie 1x01 grupots 720p.mkv")
        assert not is_trusted_provider("Serie 1x01 xgrupots 720p.mkv")
        assert not is_trusted_provider("Serie 1x01 grupots2 720p.mkv")
        assert is_trusted_provider("Serie 1x01 [MiGrupo] 720p.mkv")
    finally:
        set_provider_lists(trusted=list(_BUILTIN_TRUSTED), blocked=[])
    assert is_trusted_provider("Serie 1x01 grupots 720p.mkv")
...
def test_user_lists_invalid_entries_ignored():
    """Vacíos/duplicados/comentarios se limpian; un string suelto vale
    como un grupo; tras resetear, todo vuelve a la base."""
    from core.download_quality import (
        set_provider_lists, is_trusted_provider, is_user_blocked,
        parse_provider_lines, _BUILTIN_TRUSTED)
    assert parse_provider_lines(
        "# Fijos del sistema:\n# FR, ITA\n\nmigrupo\nMiGrupo\n  \n") == ["migrupo"]
    set_provider_lists(trusted=["", "  ", None, "Ok", "ok"], blocked="soloeste")
    try:
        assert is_trusted_provider("x OK y")
        assert is_user_blocked("x SoloEste y")
        assert not is_trusted_provider("x otra cosa y")
    finally:
        set_provider_lists(trusted=list(_BUILTIN_TRUSTED), blocked=[])
    assert is_trusted_provider("x grupots y")
    assert not is_user_blocked("x SoloEste y")
...
def test_typical_sin_mayoria_usa_mediana_global():
    """Stuart T01 real: reparto disperso {2:2, 3:1, 5:2, 6:1, 9:1} sin ningún
    bucket con mayoría. La mediana global (~1042MB) es más fiel que el bucket
    más pequeño (que daba 470MB con solo 2 ficheros de 7)."""
    sizes = [int(x * MB) for x in (452, 531, 781, 1042, 1081, 1222, 1941)]
    assert _typical_from_sizes(sizes) == int(1042 * MB)


def test_typical_con_mayoria_usa_ese_bucket():
    """Los Simpson T03: todo en el bucket 2 → mediana del bucket."""
    sizes = [int(x * MB) for x in (422, 469, 475, 484, 492, 584)]
    assert _typical_from_sizes(sizes) == (int(475 * MB) + int(484 * MB)) // 2


def test_typical_vacio_es_none():
    assert _typical_from_sizes([]) is None


def test_stuart_desproporcionado_pierde_aun_con_grupots():
    """Con típico ~1042MB, un release de 2,5GB (ratio 2,4) pierde contra uno
    de tamaño coherente aunque lleve GrupoTS."""
    big = _r(1, "Stuart 1x01 [WebDL 1080p Dual][GrupoTS].mkv",
             size="2.5 GB", sources=4, complete=True)
    ok = _r(2, "Stuart 1x01 castellano 1080p x264.mkv",
            size="1.0 GB", sources=3, complete=True)
    assert score_download(big, "Stuart 1x01", typical_size=int(1042 * MB)) < \
        score_download(ok, "Stuart 1x01", typical_size=int(1042 * MB))


# ---- V.O.S. con letras separadas (real: "El Arca" 3x01 con "V O S") ----

def test_vos_con_espacios_se_excluye():
    """Caso real: El Arca 3x01 traía "V O S" con espacios y se coló porque
    _VOS_RE solo admitía puntos entre letras. Todas las variantes con
    espacios/puntos se excluyen igual que VOS."""
    for name in ("El Arca 3x01 V O S 720p x264.mkv",
                 "El Arca 3x01 V. O. S. 720p x264.mkv",
                 "El Arca 3x01 V.O.S.E. 720p x264.mkv",
                 "El Arca 3x01 VOSE 720p x264.mkv"):
        assert is_vos_content(name), name
        assert best_result([_r(1, name)], "El Arca 3x01") is None, name


def test_vos_no_falsea_palabras_normales():
    """La guarda inicial evita que títulos con "vos" dentro ("objetivos",
    "nuevos", "vivos") se marquen como V.O.S. y queden excluidos con 0.0."""
    for name in ("Serie 1x01 Los objetivos 720p.mkv",
                 "Serie 1x01 Nuevos comienzos 720p.mkv",
                 "Serie 1x01 Vivos por los pelos 720p.mkv",
                 "Serie 1x01 Conversión original 720p.mkv"):
        assert not is_vos_content(name), name
        assert score_download(_r(1, name), "Serie 1x01") > 0.0, name


def test_vos_tras_guion_bajo_sigue_cazando():
    """SUB_VOS y VOS_x264 son etiquetas de verdad: el "_" no protege."""
    assert is_vos_content("Serie 1x01 SUB_VOS x264.mkv")
    assert is_vos_content("Serie 1x01 VOS_x264.mkv")


def test_vos_explicito_con_spanish_subs_se_excluye():
    """Caso real 2026-09-18: el autocompletado descargó 'Star Trek Strange
    New Worlds 4x09 ... (V.O.S. Spa-Eng) - Spanish subs integrados by
    JuAnItO' como si fuera castellano. 'Spanish/subs' describe muchas
    veces solo los subtítulos, así que la etiqueta VOS explícita manda."""
    name = ("Star Trek Strange New Worlds (2022) 4x09 1080p - Once La'An a "
            "Time (V.O.S. Spa-Eng) - Spanish subs integrados by JuAnItO.mkv")
    assert is_vos_content(name)
    assert best_result([_r(1, name)], "Star Trek Strange New Worlds 4x09") is None
    for variant in ("Serie 1x01 VOS Spanish subs 720p.mkv",
                    "Serie 1x01 (V.O.S.) [Spanish] 720p.mkv",
                    "Serie 1x01 VOSE subtitulada castellano.mkv",
                    "Serie 1x01 version original subtitulada spanish subs.mkv"):
        assert is_vos_content(variant), variant
        assert best_result([_r(1, variant)], "Serie 1x01") is None, variant


def test_vos_explicito_con_dual_sigue_elegible():
    """Dual = doble audio real: aunque el nombre traiga etiqueta VOS, se
    mantiene como candidato de emergencia (igual que is_italian_only con
    dual ENG-SPA)."""
